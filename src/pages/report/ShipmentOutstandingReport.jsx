...
function ShipmentOutstandingReport() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('/api/reports/shipment-outstanding');
    ...
    const filteredData = data.filter(item =>
        ...
        (item.so_number || '').toLowerCase().includes(filter.toLowerCase())
    );
    ...
    return (
        <div className="report-page">
            <div className="page-header">
                <div>
                    <h1 className="page-title">Laporan Shipment Outstanding</h1>
                    <p className="text-subtitle">Daftar Shipment yang belum dibuatkan AR Invoice</p>
                </div>
            ...
            <div className="card" style={{ marginBottom: '1rem' }}>
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <input
                        type="text"
                        placeholder="Cari No. Shipment, Customer, atau SO..."
                        value={filter}
                                ...
                                {filteredData.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" style={{ textAlign: 'center', padding: '2rem' }}>
                                            Tidak ada Shipment yang outstanding
                                        </td>
                                    </tr>
                                ) : (
...
export default ShipmentOutstandingReport;
