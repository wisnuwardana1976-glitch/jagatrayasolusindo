...
async function createReceivingTables() {
    try {
        const pool = await odbc.pool(connectionString);
        const conn = await pool.connect();

        console.log('Creating Receiving tables...');
        ...
        try {
            ...
            console.log('ℹ️ ReceivingDetails error: ' + (e.odbcErrors ? e.odbcErrors[0].message : e.message));
        }
